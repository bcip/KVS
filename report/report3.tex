...
\begin{algorithm}
	\begin{algorithmic}
		...
		\Procedure{toXML}{}
			...
			\State \texttt{root} $\leftarrow$ \texttt{createElement(KVMessage)}
			\State \texttt{doc.appendChild(root)}
			\If{msgType equals resp}
				\If{key is not null}
					\State \texttt{root.appendChild(createElement(Key))}
				\EndIf
				\If{value is not null}
					\State \texttt{root.appendChild(createElement(Value))}
				\EndIf
				\If{message is not null}
					\State \texttt{root.appendChild(createElement(Message))}
				\EndIf
			\Else
				\State \texttt{doc.appendChild(createElement(Key))}
				\If{msgType equals putreq}
					\State \texttt{doc.appendChild(createElement(Value))}
			...
			\State transform \texttt{transformer} into \texttt{String} as \texttt{retStr}
